...
function useTextStream({
  ...
  const startStreaming = useCallback(() => {
    ...
    if (typeof textStream === "string") {
      if (modeRef.current === "fade") {
        // For fade mode on a complete string, render all text immediately
        // and let CSS animation-delay handle the staggered fading
  ...
  }, [textStream, reset, processStringTypewriter, processAsyncIterable, updateSegments, markComplete])
...
export type ResponseStreamProps = {
  ...
  renderer?: (text: string) => React.ReactNode // Custom renderer for typewriter mode
}
...
function ResponseStream({
  ...
  const fadeStyle = `
    @keyframes responseStreamFadeIn {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    ...
    .fade-segment {
      ...
      opacity: 0;
      animation: responseStreamFadeIn ${getFadeDuration()}ms ease-out forwards;
    }
  ...
  const renderContent = () => {
    switch (mode) {
      case "typewriter":
        return renderer ? renderer(displayedText) : <>{displayedText}</>
